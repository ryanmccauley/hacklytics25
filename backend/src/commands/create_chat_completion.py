...
async def create_challenge_chat_completion(
  ...
  system_message = create_system_message(challenge)

  model = ChatOpenAI(model="gpt-4o", api_key=global_settings.OPENAI_API_KEY)
  messages = [system_message] + format_messages(command.messages)
...
def format_messages(messages: List[ChatMessageRequest]) -> List[BaseMessage]:
  formatted_messages = []

...
async def list_messages(query: ListMessagesQuery):
  challenge = await engine.find_one(Challenge, Challenge.id == ObjectId(query.challenge_id))
  if challenge is None:
    raise HTTPException(status_code=404)

  messages = await engine.find(ChatMessage, ChatMessage.challenge_id == ObjectId(query.challenge_id), sort=ChatMessage.created_at.asc())
